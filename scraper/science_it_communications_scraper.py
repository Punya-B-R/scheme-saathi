"""
Science, IT & Communications detail scraper.

Reads URLs from `data/science_it_communications_urls.json`, scrapes each scheme page,
and writes to data/from_urls/science_it_communications/.
"""
...
CATEGORY_NAME = "Science, IT & Communications"
INPUT_PRIMARY = Path("data/science_it_communications_urls.json")
OUT_DIR = Path("data/from_urls/science_it_communications")
CHECKPOINT_DIR = Path("checkpoints")
LOG_DIR = Path("logs")
SCHEME_ID_PREFIX = "SIC"
...
def _save_checkpoint(
    ...
    path = CHECKPOINT_DIR / f"science_it_communications_checkpoint_{idx}.json"
...
def _load_latest_checkpoint(total_urls: int) -> Tuple[set[int], int]:
    ck_files = sorted(CHECKPOINT_DIR.glob("science_it_communications_checkpoint_*.json"))
    if not ck_files:
        return set(), -1
    latest = ck_files[-1]
...
def run_scraper(test_mode: bool = False, resume: bool = False) -> None:
    _ensure_dirs()
    log_path = LOG_DIR / "science_it_communications_scraping.log"
    logging.basicConfig(
        level=logging.INFO,
    ...
    with (OUT_DIR / "science_it_communications_schemes.json").open("w", encoding="utf-8") as f:
    ...
    with (OUT_DIR / "science_it_communications_failed_urls.json").open(
        "w", encoding="utf-8"
    ) as f:
    ...
    with (OUT_DIR / "science_it_communications_stats.json").open("w", encoding="utf-8") as f:
