"""
Public Safety, Law & Justice detail scraper.
...
def _load_urls() -> List[str]:
    """Load URL list from primary or alternate input file."""
    if INPUT_PRIMARY.exists():
        path = INPUT_PRIMARY
    elif INPUT_ALT.exists():
        path = INPUT_ALT
    else:
        raise FileNotFoundError(
            f"Neither {INPUT_PRIMARY} nor {INPUT_ALT} exists. Provide a Public Safety URL JSON."
        )
...
def _save_checkpoint(schemes: List[Dict[str, Any]], processed_indices: List[int], idx: int) -> None:
    ...
    path = CHECKPOINT_DIR / f"public_safety_checkpoint_{idx}.json"
...
def _load_latest_checkpoint(total_urls: int) -> Tuple[set[int], int]:
    """
    Return (processed_indices_set, last_index_processed).
    If no checkpoint, returns (empty set, -1).
    """
    ck_files = sorted(CHECKPOINT_DIR.glob("public_safety_checkpoint_*.json"))
    if not ck_files:
        return set(), -1
    latest = ck_files[-1]
    ...
    logger.info("Resuming from checkpoint %s (last index %s of %s)", latest, last_idx, total_urls)
...
def run_scraper(test_mode: bool = False, resume: bool = False) -> None:
    ...
    # Configure logging
    log_path = LOG_DIR / "public_safety_scraping.log"
    logging.basicConfig(
        level=logging.INFO,
    ...
    if test_mode:
        urls = urls[:5]
    total = len(urls)
    logger.info("Loaded %s Public Safety URLs", total)
    ...
    try:
        ...
        for idx, url in enumerate(urls):
            ...
            while attempts < 3 and not success:
                attempts += 1
                try:
                    scheme = extract_scheme(driver, url)
    ...
    with (OUT_DIR / "public_safety_schemes.json").open("w", encoding="utf-8") as f:
    ...
    with (OUT_DIR / "public_safety_failed_urls.json").open("w", encoding="utf-8") as f:
    ...
    with (OUT_DIR / "public_safety_stats.json").open("w", encoding="utf-8") as f:
