    ...
    def create_system_prompt(
        ...
        if not has_schemes:
            ...
            parts.extend([
                ...
                "",
                "You have enough info and matching schemes. The same schemes will be shown to the user in SCHEME CARDS below your message (cards show name, benefits, eligibility, Apply link).",
                "",
                "Your message AND the cards together:",
                "- In your message: give a short friendly intro (1-2 sentences), then list each scheme by name with its apply link in markdown: [Scheme Name](url). Use the exact Website URL from the list below. Keep the message brief (under 150 words).",
                "- Do NOT repeat long benefit/eligibility text in the message — the scheme cards below will display that. Your job is intro + scheme names with links so the user sees both your message and the cards.",
                "",
                "CRITICAL RULES:",
                "- ONLY recommend schemes from the list below. NEVER invent schemes.",
                "- Mention EVERY scheme from the list that matches the user (same schemes will appear in the cards). For EACH scheme you MUST include a clickable link: write the exact markdown [Apply here](URL) or [Scheme Name](URL) using the Website URL from the list below — the user must be able to click through.",
                "- Use bullet points. Be concise. No emojis.",
                "",
